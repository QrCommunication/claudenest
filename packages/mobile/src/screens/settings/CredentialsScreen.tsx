...
const AddCredentialModal: React.FC<AddModalProps> = ({
  visible,
  onClose,
  onSubmit,
  isSubmitting,
  ...
  const handleSubmit = () => {
    ...
    if (!/^[a-z0-9-]+$/.test(name)) {
      showAlert("Error", "Use only lowercase letters, numbers, and dashes");
      return;
    }
    ...
    <Modal
      ...
      footer={
        <View style={modalStyles.footer}>
          ...
          <TouchableOpacity
            style={[modalStyles.btnPrimary, isSubmitting && { opacity: 0.5 }]}
          ...
          >
            {isSubmitting ? (
              ...
              <Text style={modalStyles.btnPrimaryText}>Create</Text>
            )}
          </TouchableOpacity>
        </View>
      }
    >
      <ScrollView style={modalStyles.body} keyboardShouldPersistTaps="handled">
        ...
        <Text style={modalStyles.label}>
          Name <Text style={{ color: colors.semantic.error }}>*</Text>
        </Text>
        <TextInput
        ...
        <View style={modalStyles.tabs}>
          <TouchableOpacity
            style={[
              modalStyles.tab,
              authType === "api_key" && modalStyles.tabActive,
            ]}
            onPress={() => setAuthType("api_key")}
          >
            <Icon
              name="vpn-key"
              size={16}
              color={
                authType === "api_key" ? colors.text.primary : colors.text.muted
              }
            />
            <Text
              style={[
                modalStyles.tabText,
                authType === "api_key" && modalStyles.tabTextActive,
              ]}
            >
              API Key
            </Text>
          ...
          <TouchableOpacity
            style={[
              modalStyles.tab,
              authType === "oauth" && modalStyles.tabActive,
            ]}
            onPress={() => setAuthType("oauth")}
          >
            <Icon
              name="verified-user"
              size={16}
              color={
                authType === "oauth" ? colors.text.primary : colors.text.muted
              }
            />
            <Text
              style={[
                modalStyles.tabText,
                authType === "oauth" && modalStyles.tabTextActive,
              ]}
            >
              OAuth
            </Text>
        ...
        {authType === "api_key" && (
          <>
            <Text style={[modalStyles.label, { marginTop: spacing.md }]}>
              API Key <Text style={{ color: colors.semantic.error }}>*</Text>
            </Text>
            <TextInput
              style={modalStyles.input}
              value={apiKey}
        ...
        {authType === "oauth" && (
          <View style={modalStyles.infoBox}>
            ...
            <Text style={modalStyles.infoText}>
              After creating, use "Connect" to authenticate via your Claude
              account.
            </Text>
          </View>
        )}
        ...
        {/* Dir Mode */}
        <Text style={[modalStyles.label, { marginTop: spacing.md }]}>
          Claude Directory Mode
        </Text>
        <View style={modalStyles.tabs}>
          <TouchableOpacity
            style={[
              modalStyles.tab,
              dirMode === "shared" && modalStyles.tabActive,
            ]}
            onPress={() => setDirMode("shared")}
          >
            <Text
              style={[
                modalStyles.tabText,
                dirMode === "shared" && modalStyles.tabTextActive,
              ]}
            >
              Shared
            </Text>
          ...
          <TouchableOpacity
            style={[
              modalStyles.tab,
              dirMode === "isolated" && modalStyles.tabActive,
            ]}
            onPress={() => setDirMode("isolated")}
          >
            <Text
              style={[
                modalStyles.tabText,
                dirMode === "isolated" && modalStyles.tabTextActive,
              ]}
            >
              Isolated
            </Text>
...
const CredentialCard: React.FC<{
  ...
  const needsReconnect =
    ["missing", "needs_login"].includes(credential.token_status) ||
    ...
    <View style={cardStyles.container}>
      <View style={cardStyles.header}>
        ...
        <View
          style={[
            ...
            credential.auth_type === "oauth" && cardStyles.typeBadgeOAuth,
          ]}
        >
          <Text
            style={[
              cardStyles.typeBadgeText,
              credential.auth_type === "oauth" && cardStyles.typeBadgeTextOAuth,
            ]}
          >
      ...
      <View style={cardStyles.body}>
        <View style={cardStyles.row}>
          ...
          <View style={cardStyles.statusRow}>
            <View
              style={[cardStyles.statusDot, { backgroundColor: statusColor }]}
            />
            <Text style={[cardStyles.rowValue, { color: statusColor }]}>
              {statusText}
            </Text>
          </View>
        </View>
          ...
          <View style={cardStyles.row}>
            ...
            <Text
              style={[
                cardStyles.rowValue,
                {
                  color: credential.has_refresh_token
      ...
      <View style={cardStyles.actions}>
        ...
        {credential.auth_type === "oauth" && needsReconnect && (
          <TouchableOpacity
            style={[cardStyles.actionBtn, cardStyles.connectBtn]}
            onPress={onConnect}
            disabled={isActionLoading}
          >
...
export const CredentialsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleCreate = useCallback(
    async (data: Parameters<typeof createCredential>[0]) => {
      ...
      try {
        ...
        showAlert(
          "Error",
          e?.response?.data?.error?.message ?? "Failed to create credential",
        );
      } finally {
  ...
  const handleConnect = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        showAlert("OAuth Failed", e?.message ?? "Connection failed");
      } finally {
        setActionLoadingId(null);
      }
    },
  ...
  if (isLoading && credentials.length === 0)
    return <LoadingSpinner text="Loading credentials..." fullScreen />;
      ...
      {credentials.length === 0 && !isLoading ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>
            Add your Claude API key or connect via OAuth
          </Text>
          <TouchableOpacity
            style={styles.addBtn}
            onPress={() => setShowAddModal(true)}
...
const modalStyles = StyleSheet.create({
  ...
  body: { maxHeight: 460, paddingVertical: spacing.sm },
  label: {
    fontSize: 13,
    fontWeight: "500",
