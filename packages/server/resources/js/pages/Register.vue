  ...
  <AuthShell>
    <div class="auth-card">
      <div class="auth-heading">
        ...
        <p class="auth-subtitle">{{ t('auth.sign_up_description') }}</p>
      </div>

      <div class="oauth-row">
        <button type="button" class="oauth-btn" @click="registerWithOAuth('google')">
        ...
        <button type="button" class="oauth-btn" @click="registerWithOAuth('github')">
      ...
      <form class="auth-form" @submit.prevent="handleSubmit" novalidate>
        <div class="field">
          <label for="register-name" class="field-label">{{ t('common.name') }}</label>
          <input
            id="register-name"
            v-model="form.name"
            type="text"
        ...
        <div class="field">
          <label for="register-email" class="field-label">{{ t('auth.email') }}</label>
          <input
            id="register-email"
            v-model="form.email"
            type="email"
        ...
        <div class="field">
          <label for="register-password" class="field-label">{{ t('auth.password') }}</label>
          <input
          ...
          <div v-if="form.password" class="strength">
            <div class="strength-bars">
              <span v-for="n in 4" :key="n" class="strength-bar" :class="{ 'is-active': n <= passwordStrength }" :data-level="passwordStrength" />
            </div>
            <span class="strength-label" :data-level="passwordStrength">{{ strengthLabel }}</span>
          </div>
          <span v-if="getFieldError('password')" class="field-error">{{ getFieldError('password') }}</span>
        </div>

        <div class="field">
          <label for="register-confirm" class="field-label">{{ t('auth.password_confirm') }}</label>
          <input
            id="register-confirm"
        ...
        <button type="submit" class="btn-primary" :disabled="!form.agreeTerms || authStore.isLoading">
          <span v-if="!authStore.isLoading">{{ t('auth.sign_up_button') }}</span>
      ...
      <p class="auth-footer-text">
        {{ t('auth.have_account') }}
        <router-link to="/login" class="form-link">{{ t('auth.sign_in_button') }}</router-link>
      </p>
    </div>
  </AuthShell>
</template>
...
.strength-bar.is-active[data-level='1'] { background: var(--status-error); }
.strength-bar.is-active[data-level='2'] { background: var(--status-warning); }
.strength-bar.is-active[data-level='3'] { background: #10b981; }
.strength-bar.is-active[data-level='4'] { background: var(--status-success); }
...
.strength-label[data-level='1'] { color: var(--status-error); }
.strength-label[data-level='2'] { color: var(--status-warning); }
.strength-label[data-level='3'] { color: #10b981; }
.strength-label[data-level='4'] { color: var(--status-success); }
